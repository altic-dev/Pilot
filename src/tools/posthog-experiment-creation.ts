...
export const posthogExperimentCreationTool = tool({
  ...
  execute: async ({ projectId, name, description, featureFlagKey }) => {
    ...
    if (!response.ok) {
      logger.error("Unable to create experiment", response);
      throw new Error(
        `Unable to create experiment, PostHog API error: ${response.status}`,
      );
    }
